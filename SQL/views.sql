...
DROP VIEW football.top_scorers;
CREATE VIEW football.top_scorers AS SELECT RANK() OVER(ORDER BY (SELECT COUNT(*) FROM football.bramki b WHERE pilkarz.id_pilkarza = b.id_pilkarza)) AS pozycja, pilkarz.imie, pilkarz.nazwisko, 
...
DROP VIEW football.zawieszenia;
CREATE VIEW football.zawieszenia AS SELECT (SELECT d.nazwa FROM football.druzyna d WHERE d.id_druzyny = pilkarz.id_druzyny) AS druzyna, pilkarz.imie, pilkarz.nazwisko,
    CASE
        ...
        WHEN pilkarz.mecze_zawieszenia = 0 THEN 'Niedostepny do wpisania, ostatni mecz zawieszenia'
